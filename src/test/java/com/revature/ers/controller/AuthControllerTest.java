...
class AuthControllerTest {
     ...
     * is what actually proves Step 1 (validate) and Step 2 (issue) fit together. Uses /requests/{id}
     * (authenticated, any role) rather than /requests, which is now Supervisor-only - role rules
     * are exercised separately in AuthorizationRulesTest.
     */
    @Test
    void mintedToken_isAcceptedOnSecuredRoute() throws Exception {
        String token = tokenService.mint(employeeWithPassword("irrelevant"));

        mockMvc.perform(get("/requests/1").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk());
    }
}
